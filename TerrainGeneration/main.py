...
buildMode = -1  # -1 Disabled, 1 Enabled
...
def input(key):
    ...
    build_distance += 1 if key == 'scroll up' else -1 if key == 'scroll down' else 0

    quit() if key == 'q' or key == 'escape' else None

    (generating, canGenerate) = (generating * -1, canGenerate * -1) if key == 'g' else (generating, canGenerate)
